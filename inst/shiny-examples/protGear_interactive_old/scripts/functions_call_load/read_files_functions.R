...
#'
#' @examples
plot_bg <- function(df, antigen_name="antigen",bg_MFI="B635_Median",
                    log_mfi=T){
 ...
 # bg_plot$sampleID <- group_indices(.data =bg_plot )
  ...
  if(log_mfi==T){
    ...
    p_bg <- ggplot(data = bg_plot ,aes_string(  x=antigen_name , y="log_bg"))+
      geom_boxplot()+
    ...
    p_pubr <- ggboxplot(data = bg_plot , 
              x=antigen_name , y=bg_MFI,
              facet.by = "replicate",ncol=1)
    p_pubr <- ggpar(p_pubr,font.tickslab = c(6,"#993333"),
          xtickslab.rt = 45 , ylab = "Background of the replicates raw") 
   
    p_bg <- ggplot(data = bg_plot ,aes_string(  x=antigen_name , y=bg_MFI))+
      geom_boxplot()+
      facet_wrap(~replicate, ncol=1) +
      theme_light() +
      theme(axis.text.x = element_text(angle = 45 , hjust = 1)) 
    ...
    p_bg <- ggplot(data = bg_plot ,aes_string(  x="antigen" , y=bg_MFI))+
      geom_boxplot()+
      facet_wrap(~replicate, ncol=1) +
      theme_light() +
      theme(axis.text.x = element_text(angle = 45 , hjust = 1)) 
