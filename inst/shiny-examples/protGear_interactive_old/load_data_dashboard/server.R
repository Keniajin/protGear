  ...
  if(!is.null(gpr_file_reactive)){
    ...
    tagList(
      ...
      numericInput("blockspersample_param",
        "Input the number mini arrays or blocks per sample as per the graph below",
        value = value_check
      )
    )
  }
...
output$bg_plots <- renderPlotly({
  if(input$chip_path_param!="") {
    ...
    p<- plot_bg(df=allData_bg, antigen_name=input$block_antigen,bg_MFI="BG_Median",
...
observe({ # called only once at app initilisation
...
normalised_list_all_reactive <- reactive({
 ...
 # antigens <- antigens[antigens %in% names(to_normalise)]
  ...
  if(input$rlm==T){
    normalise_list_cyclic_rlm <- matrix_normalise(matrix_antigen=matrix_antigen, 
                                                  method = "rlm", 
                                                  array_matrix=array_matrix,
                                                  return_plot = T,
                                                  control_antigens=control_antigens)
    names(normalise_list_cyclic_rlm) <- c("matrix_antigen_rlm" ,"plot_rlm")
    
  }else if(input$rlm==F){
    p <- empty_plot_NS() + ggtitle("RLM")
    normalise_list_cyclic_rlm <- list(plot_rlm=p, matrix_antigen_rlm=NULL)
  }
  
  normalise_list <- c(normalise_list_log,normalise_list_vsn,normalise_list_cyclic_loess_log,
                      normalise_list_cyclic_loess,normalise_list_cyclic_rlm)
  return(normalise_list)
