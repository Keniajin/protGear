...
matrix_normalise <- function(matrix_antigen, method="log2",batch_correct=F,batch_var1=machines,batch_var2=day_batches,
                             return_plot=F,control_antigens=NULL, array_matrix=NULL ,plot_by_antigen=T){
  ...
  if(method=="log2"){
    ...
    print("exprs_log",head(exprs_normalised))
  } else if(method=="none"){
...
plot_normalised <- function(exprs_normalised_df,method,batch_correct){
  ...
  cs_trend <- trend::cs.test(exprs_normalised_df_plot$stdev_all_anti , alternative = "greater")
  ...
  p_norm <- ggplot(exprs_normalised_df_plot ,  aes(x=rank_mean_all_anti, y=stdev_all_anti)) +
    geom_jitter(color="red") + theme_classic() + 
    expand_limits(y=c(0,10))+  stat_cor() +
    geom_smooth(color='blue',se = F, size=0.5)+
