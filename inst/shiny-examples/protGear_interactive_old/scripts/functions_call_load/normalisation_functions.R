...
#'
#' @examples
matrix_normalise <- function(matrix_antigen, method="log2",batch_correct=F,batch_var1=machines,batch_var2=day_batches,
                             return_plot=F,control_antigens=NULL, array_matrix=NULL){
  if(method=="log2"){
    ...
    ## this approach utlises the VSN package
    ## structure the data to be normalised
    ## convert it to an Expression Set
    data_points <- dim(matrix_antigen)[[1]]
    exprs_antigen <-  
      Biobase::ExpressionSet(assayData = t(matrix_antigen))
    
    if(batch_correct==F){
      ## normalise without adjusting for the strata on VSN
      exprs_normalised <-  vsn::justvsn(x=exprs_antigen ,minDataPointsPerStratum=data_points)
      
    }else if(batch_correct==T){
      exprs_normalised <-
        vsn::justvsn(x = exprs_antigen , strata = machines,minDataPointsPerStratum=data_points)
    }
  ...
  plot_normalisation <- plot_normalised(exprs_normalised_df=exprs_normalised_df,
                                        method=method,batch_correct=batch_correct,cv_val)
  
  ## return the data in the structure as it was supplied
  if(method=="vsn"){
    matrix_antigen_normalised <- t(exprs_normalised_df)
  ...
  ## specify whether to include plot on return
  if(return_plot==T){
...
plot_normalised <- function(exprs_normalised_df,method,batch_correct,cv_val){
  ...
  p_norm <- ggplot(exprs_normalised_df_plot ,  aes(x=rank_mean_all_anti, y=stdev_all_anti)) +
    ...
    ggtitle(paste(norm_method,"Normalisation with a pooled CV of ",cv_val)) + xlab("Mean rank normalised") + 
    ylab("SD Normalized") 
  return(p_norm)
}
